---
name: agent-birth
description: Bootstrap, birth, inspect, repair, or reconfigure AI Agent Tool in the current Gemini CLI workspace. Use when the user invokes @agents or /agent-birth, asks for a persistent project agent, runs agent status or doctor, changes portable memory mode, or asks the project agent to remember or forget durable context. Do not use for ordinary project work.
---

# Bootstrap or maintain the project agent

1. Treat the directory containing this `SKILL.md` as the trusted adapter source.
2. If `.ai-agent/BIRTH.md`, `.ai-agent/STATE.md`, the marked AI Agent Tool block in root `GEMINI.md`, `.gemini/agents/ai-agent-tool.md`, or `.gemini/commands/agent-birth.toml` is absent, read `references/BOOTSTRAP.md` completely and run its additive installation or repair workflow first.
...
5. Follow the matching protocol exactly and keep every write inside the current workspace.

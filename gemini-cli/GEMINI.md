...
The workspace defines a local subagent named `agents`. When the user starts a prompt with `@agents`, delegate immediately to that subagent. `/ai-agent:init` is the deterministic custom-command fallback.
